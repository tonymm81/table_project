...
def save_config(save_key: str, value: dict, slot_index: int = None): # version 133 changes
    conn = get_connection() 
    cur = conn.cursor() 
    try: 
        cur.execute( "REPLACE INTO saved_configs (save_key, slot_index, value_json) VALUES (%s, %s, %s)", (save_key, slot_index, json.dumps(value)) ) 
        conn.commit() 
    except Exception: 
        conn.rollback() 
        raise 
...
def load_config(save_key: str): # version 133 changes
    conn = get_connection() 
    cur = conn.cursor()
    try: 
        ...
        raw = row[0] # tuple-muoto, kuten vanhassa koodissa 
        try: 
