...
def loadSavedSettingsFromPhone(devices):
    global savename
...
def load_settings(devices):
    ...
    load_btn1 = Button(top4, text=str(savename[0]),fg="white", bg="black",font=("helvetica", 15), command=lambda: return_wlan_devices(savename[0], devices)).grid(row = 3, column=1)
    load_btn2 = Button(top4, text=str(savename[1]),fg="white", bg="black",font=("helvetica", 15), command=lambda: return_wlan_devices(savename[1], devices)).grid(row = 5, column=1)
    load_btn3 = Button(top4, text=str(savename[2]),fg="white", bg="black",font=("helvetica", 15), command=lambda: return_wlan_devices(savename[2], devices)).grid(row = 7, column=1)
    load_btn4 = Button(top4, text=str(savename[3]),fg="white", bg="black",font=("helvetica", 15), command=lambda: return_wlan_devices(savename[3], devices)).grid(row = 9, column=1)
    top4.mainloop()
    return
...
def get_user_data_from_phone(entry, measure):
    global savename
    ...
    with open(f"{entry}.json", "w", encoding="utf-8") as outfile:
...
def return_wlan_devices_from_phone(saveslot, devices): # here we open new and saved json and measure table distance from loaded json value
    #wlan_devices.update_json(devices)
    new_json = wlan_devices.get_json() # this is new. based on start time
    new_json_temp = json.loads(new_json) # loads convert to python dictonary and load only json string.
    size = len(saveslot)
